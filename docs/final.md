...
The logging level helps filter out scores that provide less insight into the algorithm's performance. For example, the genetic algorithm logs all scores for all strings at level 1, but also logs the scores for the best-scoring strings within each population at level 0. This helps us distinguish between an algorithm's ability to locate good strings, and the algorithm's performance when it is spending time in a more exploratory phase. The logging behaviour for all algorithms is as follows:

* **Genetic Algorithm:** The genetic algorithm logs every score at level 1, and the scores for the highest-scoring members of each generation at level 0.
* **Hill-Climbing / Annealing Algorithms:** The hill-climbing algorithms log every score at level 1, and the scores for each string that causes them to switch their `current_string` at level 0.
* **Brute Force Algorithm:** The brute force algorithm logs every score at level 1, and the scores for the best string of a given length at level 0.
